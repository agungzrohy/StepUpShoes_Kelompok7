...
import 'package:store_app/views/payment_page.dart';
...
class _CartProductsState extends State<CartProducts> {
  ...
  void _deleteCartProduct(CartModel deletedProduct) {
    setState(() {
      int index =
          cartProducts.indexWhere((product) => product.id == deletedProduct.id);
      cartProducts.removeAt(index);
    });
  }

  int _getTotalPrice() {
    List<int> priceList = cartProducts
        .map((product) => product.price * product.ammounts)
        .toList();
    int totalPrice = 0;
    if (priceList.isNotEmpty) {
      totalPrice =
          priceList.reduce((accumulator, price) => accumulator + price);
    }
    return totalPrice;
  }
  ...
  Widget build(BuildContext context) {
    return CupertinoPageScaffold(
      ...
      children: [
        Expanded(
          child: Column(children: [heading(), body()]),
        ),
        footer()
      ],
    )));
    ...
    return Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
      ...
      Column(children: [
        ...
        Text('${cartProducts.length} barang',
            textAlign: TextAlign.center,
            style: GoogleFonts.sourceSans3(
                fontSize: 20.0, color: const Color(0xff595959))),
      ...
      children: [
        ...
        Padding(
          ...
          child:
            ...
            const Text('Total'),
            Text('IDR ${PriceUtils.formatPrice(_getTotalPrice())} K',
        ...
        CupertinoButton(
          ...
          onPressed: () {
            Navigator.push(
              context,
              CupertinoPageRoute(
                builder: (context) => PaymentPage(cartProducts: cartProducts),
              ),
            );
          },
        )
